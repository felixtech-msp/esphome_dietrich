  ...
  void getSample() {
    ...
    if (isValidCRC(readdata, n)) {
      ...
      if (!flow_temp_sensor->get_name().empty())
        flow_temp_sensor->publish_state(signedFloat((readdata[8] * 256) + readdata[7]) * 0.01);
      delay(100); // delay for esphome to not disconnect api

      if (!return_temp_sensor->get_name().empty())
        return_temp_sensor->publish_state(signedFloat((readdata[10] * 256) + readdata[9]) * 0.01);
      delay(100); // delay for esphome to not disconnect api

      if (!dhw_in_temp_sensor->get_name().empty())
        dhw_in_temp_sensor->publish_state(signedFloat((readdata[12] * 256) + readdata[11]) * 0.01);
      delay(100); // delay for esphome to not disconnect api

      if (!outside_temp_sensor->get_name().empty())
        outside_temp_sensor->publish_state(signedFloat((readdata[14] * 256) + readdata[13]) * 0.01);
      delay(100); // delay for esphome to not disconnect api

      if (!calorifier_temp_sensor->get_name().empty())
        calorifier_temp_sensor->publish_state(signedFloat((readdata[16] * 256) + readdata[15]) * 0.01);
      delay(100); // delay for esphome to not disconnect api

      if (!boiler_control_temp_sensor->get_name().empty())
        boiler_control_temp_sensor->publish_state(signedFloat((readdata[20] * 256) + readdata[19]) * 0.01);
      delay(100); // delay for esphome to not disconnect api

      if (!room_temp_sensor->get_name().empty())
        room_temp_sensor->publish_state(signedFloat((readdata[22] * 256) + readdata[21]) * 0.01);
      delay(100); // delay for esphome to not disconnect api

      if (!ch_setpoint_sensor->get_name().empty())
        ch_setpoint_sensor->publish_state(signedFloat((readdata[24] * 256) + readdata[23]) * 0.01);
      delay(100); // delay for esphome to not disconnect api

      if (!dhw_setpoint_sensor->get_name().empty())
        dhw_setpoint_sensor->publish_state(signedFloat((readdata[26] * 256) + readdata[25]) * 0.01);
      delay(100); // delay for esphome to not disconnect api

      if (!room_temp_setpoint_sensor->get_name().empty())
        room_temp_setpoint_sensor->publish_state(signedFloat((readdata[28] * 256) + readdata[27]) * 0.01);
      delay(100); // delay for esphome to not disconnect api
      
      if (sem_read_all) {
        if (!fan_speed_setpoint_sensor->get_name().empty())
          fan_speed_setpoint_sensor->publish_state(signedFloat((readdata[30] * 256) + readdata[29]));
        delay(100); // delay for esphome to not disconnect api

        if (!fan_speed_sensor->get_name().empty())
          fan_speed_sensor->publish_state(signedFloat((readdata[32] * 256) + readdata[31]));
        delay(100); // delay for esphome to not disconnect api
        ...
        if (!internal_setpoint_sensor->get_name().empty())
          internal_setpoint_sensor->publish_state(signedFloat((readdata[35] * 256) + readdata[34]) * 0.01);
        delay(100); // delay for esphome to not disconnect api
      ...
      lockout_sensor->publish_state(readdata[48]);
      delay(200); // delay for esphome to not disconnect api

      blocking_sensor->publish_state(readdata[49]);
      delay(200); // delay for esphome to not disconnect api
